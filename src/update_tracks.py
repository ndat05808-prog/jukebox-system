...
class UpdateTracks:
    def __init__(self, window):
        ...
        ttk.Label(window, text="Update Track Information", style="Hero.TLabel").grid(row=0, column=0, columnspan=2, sticky="w", padx=18, pady=(18, 6))
        ttk.Label(window, text="Edit metadata while keeping the original coursework logic.", style="Muted.TLabel").grid(row=0, column=0, columnspan=2, sticky="w", padx=18, pady=(56, 12))
        ...
        ttk.Label(search_card, text="Load Track", style="CardTitle.TLabel").grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 12))
        ...
        ttk.Button(search_card, text="Load", style="Neon.TButton", command=self.load_track_clicked).grid(row=1, column=2, sticky="ew")
        ...
        ttk.Label(form_card, text="Edit Form", style="CardTitle.TLabel").grid(row=0, column=0, columnspan=2, sticky="w", pady=(0, 12))

        ttk.Label(form_card, text="Name", style="Card.TLabel").grid(row=1, column=0, sticky="w")
        ...
        ttk.Label(form_card, text="Album", style="Card.TLabel").grid(row=3, column=0, sticky="w")
        ...
        ttk.Label(form_card, text="Rating", style="Card.TLabel").grid(row=5, column=0, sticky="w")
        ...
        ttk.Button(form_card, text="Update Track", style="Neon.TButton", command=self.update_track_clicked).grid(row=6, column=1, sticky="ew", pady=(6, 12))
        ...
        ttk.Label(preview_card, text="Track Preview", style="CardTitle.TLabel").grid(row=0, column=0, sticky="w", pady=(0, 10))
    ...
    def list_tracks_clicked(self):
        ...
        for record in lib.get_track_records():
            self.tree.insert("", "end", values=(record["key"], record["name"], record["artist"], record["album"] or "—", stars_text(record["rating"])))
        self.status_lbl.configure(text="Library tracks are displayed.")

    def _clear_edit_fields(self):
        for widget in (self.name_input, self.artist_input, self.album_input, self.year_input, self.track_input):
